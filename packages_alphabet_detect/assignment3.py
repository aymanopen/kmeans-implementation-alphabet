...
def get_images(alpha,path='Assignment 3 Dataset/'):
    alphabet=alpha
    allimages=[]
    allimages_test=[]
    for id_char,x in enumerate(alphabet):
        for filename in glob.glob(path+'A1'+str(x)+'*'):
...
def getDistancesMatrixSorted(training):
    ...
    for i in range(0,lengthoftrain):
        ...
        alldistances_sorted.append(distances[0:])
    ...
    for alphaindex,train20row in enumerate(testinglist):
        for testelement in train20row:
            ...
            for distances in distancesMatrix[testelement][:k]:
                ...
                #if distances[2]!=alphaindex:
                #    errors+=1
                #print(train20row,distances[2],distances[1])
    ...
    for testrowerror in klist:
        ...
        if len(mcommon)==1:
            ...
            if mcommon[0][0]!= testrowerror[1]: 
                error_all.append(1)
                #print (mcommon[0][0],testrowerror[1])
        else:
            ...
            if mcommon[0][1] != mcommon[1][1]:
                ...
                if mcommon[0][0]!= testrowerror[1]:
                    #error++
                    error_all.append(1)
                    #print(mcommon,testrowerror)
            else:
                #check the one closest 
                if alldistances_sorted[testrowerror[0]][0][2] != testrowerror[1]:
                    error_all.append(1)
                    #print (testrowerror[0],alldistances[testrowerror[0]][0],testrowerror[1])
